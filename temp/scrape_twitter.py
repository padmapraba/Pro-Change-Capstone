...
def getData(keyword,year):
    ...
    for day_num in days:
        ...
        # Creating a dataframe from the tweets list aboveclear
    ...
    # print(f'{year}:'+str(len(tweets_df)))
    # file = ''.join(keyword.split()).lower()
    # tweets_df.to_csv(f'data/{file}/{file}_{year}.csv',index=False)
    return tweets_df


keywords = ['climatechange','globalwarming','extremeweather','heatwave','carbonemissions','sustainable','emissions','climatescience','carbonneutral','ClimateCrisis','ClimateEmergency','ClimateScam','greenhousegas','climatechangeho']
